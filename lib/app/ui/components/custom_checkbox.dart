  ...
  Widget _buildIcon() {
    ...
    switch (_status) {
      ...
      case CheckStatus.unchecked:
        fillColor = widget.uncheckedFillColor ?? const Color(0x00222222).changeOpacity(0.1);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color:
              widget.shouldShowBorder
                  ? (widget.borderColor ?? AppColors.primary.changeOpacity(0.6))
                  : (!widget.value ? (widget.borderColor ?? AppColors.primary.changeOpacity(0.6)) : Colors.transparent),
          width: widget.shouldShowBorder ? widget.borderWidth ?? 2.0 : 1.0,
        ),
      ),
      child: Icon(iconData, color: iconColor, size: widget.checkBoxSize ?? 18),
    );
  }
